...
let write_svx ~gpx ~output ~gpx_file ~gpx_coordinate_system:_
      ~svx_time_zone ~underground_legs =
  Out_channel.fprintf output
    "; ** This file is autogenerated; do not edit **\n";
  Out_channel.fprintf output
    "; Generated by gps2survex from %s\n"
    gpx_file;
...
*)
  ...
  Out_channel.fprintf output "*data nosurvey from to\n";
  Out_channel.fprintf output "*infer exports on\n";
    ...
    List.iteri track.segments ~f:(fun segment_index segment ->
      ...
      List.iteri segment ~f:(fun station point ->
        if station > 0 then begin
          Out_channel.fprintf output "%d %d\n"
            station
            (station - 1)
        end;
  ...
  Command.basic
    ...
    [%map_open
     let gpx_coordinate_system = (* CR mshinwell: remove this *)
      ...
      and underground_legs =
        flag "underground-legs" no_arg
           ~doc:" Emit underground instead of surface legs between points"
      in
      fun () ->
